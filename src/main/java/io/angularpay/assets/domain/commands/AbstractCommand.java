...
import io.angularpay.assets.exceptions.ValidationException;
...
import static io.angularpay.assets.helpers.Helper.*;
...
public abstract class AbstractCommand<T extends AccessControl, R> {
    ...
    public R execute(T request) {
        try {
            ...
            if (this instanceof UpdatesPublisherCommand && response instanceof AssetRequestSupplier) {
                log.info("publishing {} update to REDIS => message payload:  {}", this.name, responseText);
                Executors.newSingleThreadExecutor().submit(() -> {
                    ((UpdatesPublisherCommand)this).publishUpdates((AssetRequestSupplier)response);
                });
            }
            if (this instanceof UserNotificationsPublisherCommand && response instanceof AssetRequestSupplier) {
                log.info("publishing {} User Notification to REDIS", this.name);
                Executors.newSingleThreadExecutor().submit(() -> {
                    ((UserNotificationsPublisherCommand)this).publishUserNotification((AssetRequestSupplier)response);
                });
            }
